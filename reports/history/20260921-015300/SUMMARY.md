...
- 生成时间：2026-09-21T01:39:10+08:00
...
- **[GLOBAL]** 协议双源：pc/display/compat/apx/frame.h 是 shared/include/apx/frame.h 的副本 —— 内容与真源不一致（已发生布局漂移）
- **[GLOBAL]** 协议双源：pc/host/compat/apx/frame.h 是 shared/include/apx/frame.h 的副本 —— 内容与真源不一致（已发生布局漂移）
- **[GLOBAL]** 协议双源：pc/host/compat/apx/hid_layout.h 是 shared/include/apx/hid_layout.h 的副本 —— 内容与真源不一致（已发生布局漂移）
- **[GLOBAL]** 协议双源：pc/host/compat/apx/sensors_id.h 是 shared/include/apx/sensors_id.h 的副本 —— 内容与真源不一致（已发生布局漂移）
...
> 本节记录 main 在 `docs/PROTOCOL.md` 上的每次修改原因、影响范围与受影响 lane。来源依据必须是已留痕的 main 介入记录或某路 `contractDeviations`。

### 2026-09-21 · PROTOCOL.md §5 版本字段同步至 v1.4

**变更原因**：协议实际已升至 v1.4（§2.1 实测描述符 1727B 与 §2.10 触控板 Mouse TLC 均已落地），但 §5「当前版本」字段与变更记录表停留在 1.3，字段与正文不一致，违反 §10.4「变更留痕」要求。

**影响范围**：
- §5「版本与兼容」：`当前 1.3` → `当前 1.4`
- §5 变更记录表：新增 v1.4 行（§2.10 触控板 Mouse TLC）

**受影响 lane**：
- **L1（core-proto）**：`shared/src/hid_descriptor.cpp` 的 `buildTlcMouse` 是 v1.4 新增的描述符构造器，已随真机 APK 构建一并落地（MAIN-INTERVENTIONS.md 2026-09-20 · 打通 Android 构建 一节）
- **L3（android-screen-ui）**：触控板手势识别在端侧完成，单指移动/双指滚动/双指右键/三指中键采集后通过 Report ID 2 上行
- **L4（pc-display）**：PC 端按 `Data,Var,Relative` 语义注入相对位移（与 §2.5 Digitizer 的绝对坐标注入路径互斥）
- **L5（pc-host）**：控制面板需新增触控板配置项（灵敏度曲线、加速度、惯性参数留端侧可调）

**依据**：
- `reports/MAIN-INTERVENTIONS.md` 2026-09-20 · 真机 MS1 验证（描述符已升至 v1.3，APK 含 4 ABI 的 libapx.so 构建通过）
- `docs/PROTOCOL.md` §2.1 描述符实测 1727B / §2.10 触控板布局 6 字节 + `buildTlcMouse` 描述符要点

**未变更**：本节只是字段同步与留痕，不引入新协议字段。所有 v1.4 描述符/布局改动 L1 已实现完毕，本轮仅补登记。L1 回报（`reports/L1.json`）时间戳 2026-09-20T12:57:37 早于 main 介入，未反映 v1.3/v1.4 状态，需 L1 重新回报后才能让汇总矩阵反映真实进展。
